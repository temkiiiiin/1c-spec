...
* Сравниваем партии документа и остатков: **НЕ втДанныеДокумента.ПартияДокумента = ОстаткиНоменклатурыОстатки.Партия**. Результатом сравнения является значение типа БУЛЕВО, значит, чтобы значения с совпадающими партиями обрабатывались первыми, нужно инвертировать значение инфорукцией **НЕ** (ЛОЖЬ меньше чем ИСТИНА)
